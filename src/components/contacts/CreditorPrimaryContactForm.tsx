...
interface CreditorPrimaryContactFormProps {
  companyId: string;
  creditorId: string;
...
export function CreditorPrimaryContactForm({
  companyId,
  creditorId,
  initialData,
  onSuccess,
  inline = false,
}: CreditorPrimaryContactFormProps) {
  ...
  const onSubmit = async (data: PrimaryContactInput) => {
    ...
    try {
      await creditorService.updatePrimaryContact(companyId, creditorId, {
        name: data.name,
