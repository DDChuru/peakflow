...
interface CreditorFinancialContactsManagerProps {
  companyId: string;
  creditorId: string;
  onContactsChange?: () => void;
}

export function CreditorFinancialContactsManager({
  companyId,
  creditorId,
  onContactsChange,
}: CreditorFinancialContactsManagerProps) {
  ...
  const loadContacts = async () => {
    try {
      setLoading(true);
      const data = await creditorService.getFinancialContacts(companyId, creditorId);
      setContacts(data);
    } catch (error) {
      console.error('Error loading financial contacts:', error);
      // Gracefully handle service errors
      if (error instanceof TypeError && error.message.includes('is not a function')) {
        console.warn('[CreditorFinancialContactsManager] Service method not available, using empty contacts');
        setContacts([]);
      } else {
  ...
  const handleAddContact = async (data: FinancialContactInput) => {
    ...
    try {
      await creditorService.addFinancialContact(companyId, creditorId, {
        name: data.name,
        email: data.email,
  ...
  const handleEditContact = async (data: FinancialContactInput) => {
    ...
    setIsSubmitting(true);
    try {
      await creditorService.updateFinancialContact(companyId, creditorId, selectedContact.id, {
        name: data.name,
        email: data.email,
        phone: data.phone || undefined,
        position: data.position,
  ...
  const handleDeleteContact = async () => {
    ...
    setIsSubmitting(true);
    try {
      await creditorService.removeFinancialContact(companyId, creditorId, selectedContact.id);
  ...
  const handleToggleActive = async (contact: FinancialContact) => {
    try {
      await creditorService.updateFinancialContact(companyId, creditorId, contact.id, {
  ...
  const handleSetPrimary = async (contact: FinancialContact) => {
    try {
      await creditorService.setPrimaryFinancialContact(companyId, creditorId, contact.id);
