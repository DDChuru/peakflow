...
import { createStatementService, pdfService, type StatementPDFOptions, InvoiceService } from '@/lib/accounting';
import { debtorService, companiesService } from '@/lib/firebase';
...
export default function StatementsPage() {
  ...
  const [previewStatement, setPreviewStatement] = useState<CustomerStatement | null>(null);
  ...
  async function loadData() {
    try {
      ...
      // Load invoices for real-time AR metrics
      const invoiceService = new InvoiceService();
      const invoicesData = await invoiceService.getInvoices(companyId);
  ...
  async function loadStatements() {
    try {
      ...
      setStatements(statementsData as CustomerStatement[]);
    } catch (error) {
  ...
  // Calculate real-time AR metrics from invoices
  const arMetrics = {
    ...
    customersWithBalance: new Set(
      invoices
        .filter(inv => ['sent', 'partial', 'overdue'].includes(inv.status) && (inv.amountDue || 0) > 0)
        .map(inv => inv.customerId)
    ).size,
  };

  // Statement activity metrics
  const statementMetrics = {
    sentThisMonth: statements.filter(
      s => s.status === 'sent' &&
      s.sentAt &&
      new Date(s.sentAt).getMonth() === new Date().getMonth() &&
      new Date(s.sentAt).getFullYear() === new Date().getFullYear()
    ).length,
  ...
  async function handleGenerateStatement() {
    if (!selectedCustomerId || !periodStart || !periodEnd) {
      toast.error('Please select customer and period');
      return;
    }
  ...
  // Batch generate for all customers
  async function handleBatchGenerate() {
    ...
    try {
      ...
      const statementService = createStatementService(companyId, user.uid);
      const result = await statementService.batchGenerateCustomerStatements({
        entityType: 'customer',
      ...
      toast.success(
        `Generated ${result.successful} statements (${result.failed} failed)`
      );

      await loadStatements();
    } catch (error: any) {
      console.error('Error batch generating statements:', error);
  ...
  return (
    <div className="p-8 space-y-6">
      ...
      <div className="flex justify-between items-start">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            Generate and manage professional customer statements with aged analysis
          </p>
        </div>
      ...
      {/* Summary Cards - Real-time AR Metrics */}
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card>
          <CardHeader className="pb-3">
            ...
            <CardDescription className="text-xs">From active invoices</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{formatCurrency(arMetrics.totalOutstanding)}</div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader className="pb-3">
            ...
            <CardDescription className="text-xs">Invoices past due date</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-red-600">
              {formatCurrency(arMetrics.overdueAmount)}
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader className="pb-3">
            ...
            <CardDescription className="text-xs">Unique customers</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{arMetrics.customersWithBalance}</div>
            <div className="text-xs text-gray-500 mt-1">of {customers.length} total</div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{statementMetrics.sentThisMonth}</div>
            <div className="text-xs text-gray-500 mt-1">{statementMetrics.totalGenerated} total generated</div>
          </CardContent>
        </Card>
      </div>
          ...
          <div className="flex gap-4">
            <div className="flex-1">
              <div className="relative">
                ...
                <Input
                  placeholder="Search by customer name or statement ID..."
                  value={searchTerm}
        ...
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-12">
            ...
            <p className="text-gray-600 mb-4">
              Generate your first customer statement to get started
            </p>
            <Button onClick={() => setIsGenerateDialogOpen(true)}>
              <Plus className="h-4 w-4 mr-2" />
              Generate Statement
      ...
      <Dialog open={isGenerateDialogOpen} onOpenChange={setIsGenerateDialogOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Select a customer and period to generate a statement
            </DialogDescription>
          </DialogHeader>
      ...
      <Dialog open={isPreviewDialogOpen} onOpenChange={setIsPreviewDialogOpen}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          {previewStatement && (
            <>
              <DialogHeader>
                <DialogTitle>Statement Preview - {previewStatement.customerName}</DialogTitle>
                <DialogDescription>
                ...
                <Card>
                  ...
                  <CardContent className="space-y-2">
                    ...
                    {formatAgeBucket(
                      ...
                      `Invoices (${previewStatement.summary.invoiceCount})`
                    )}
                    {formatAgeBucket(
